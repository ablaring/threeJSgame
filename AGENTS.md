# AGENTS.md — GTA-like Three.js · Part 1 : Engine & World
...
> **Codex est mauvais en modélisation 3D intuitive.**
> Chaque fois qu'un objet 3D est créé ou modifié, Codex DOIT prendre un screenshot et l'analyser avant de continuer.
...
# Puis Codex analyse l'image générée dans debug/
```

### Option B — MCP Puppeteer (si installé dans l'environnement Codex)
...
```
→ L'image est retournée directement dans le contexte de Codex, pas besoin de la sauvegarder.
...
```
Appeler `captureFrame()` dans la console du browser, puis copier-coller le screenshot dans le contexte Codex.

---

## Checklist d'analyse visuelle — ce que Codex doit vérifier sur chaque screenshot

Après chaque screenshot, Codex vérifie systématiquement :
